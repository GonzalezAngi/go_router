...
class _IsolateViewState extends State<IsolateView> {
  ...
  Future<void> isolateTask() async {
    ...
    if (!mounted) return;
    setState(() {
      resultado = "Presiona el botón para ejecutar"; 
    });
    // Muestra el resultado en un SnackBar
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(result), duration: const Duration(seconds: 30)),
    );
  }
  ...
  static void _simulacionTareaPesada(SendPort sendPort) async {
    ...
    await for (final message in port) {
      ...
      puertoReceptor.send(
        "Tarea completada. Suma del 1 al 20000: $counter. -  Mensaje recibido: '$data'",
      );
